...
from blog.views.forms import CommentForm


class CommonForm(Form):
    types = SelectField(u'文章分类', coerce=int, validators=[DataRequired()])
    source = SelectField(u'文章来源', coerce=int, validators=[DataRequired()])


class SubmitArticlesForm(CommonForm):
    title = StringField(u'标题', validators=[DataRequired(), Length(1, 64)])
    content = TextAreaField(u'文章内容', validators=[DataRequired()])
